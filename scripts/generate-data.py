...
def occupancy_multiplier(timestamp: datetime) -> float:
    day_of_year = timestamp.timetuple().tm_yday
    seasonal_multiplier = 1.0 + 0.2 * math.sin(2 * math.pi * (day_of_year - 80) / 365)
    weekend_multiplier = 1.15 if timestamp.weekday() >= 5 else 1.0
    return seasonal_multiplier * weekend_multiplier


def build_occupancy_rows(
    record_count: int, zones: list[dict[str, object]], rng: random.Random
) -> list[dict[str, object]]:
    sensor_count = max(100, record_count // 30)
    sensors = [f"OCC-{index + 1:04d}" for index in range(sensor_count)]
    start = datetime(2025, 1, 1, 0, 0, 0)

    room_cap = {
        "residential": {"min": 50, "max": 250},
        "commercial": {"min": 1000, "max": 2500},
        "industrial": {"min": 500, "max": 1250},
        "mixed_use": {"min": 100, "max": 500},
        "park": {"min": 10, "max": 50},
        "campus": {"min": 75, "max": 500},
    }
    default_cap = {"min": 10, "max": 50}

    rows: list[dict[str, object]] = []
    for index in range(record_count):
        timestamp = start + timedelta(minutes=15 * index)
        zone = zones[(index * 4) % len(zones)]
        lat, lon = random_point(zone, rng)
        zone_type = str(zone["zone_type"])
        cap = room_cap.get(zone_type, default_cap)
        if cap:
            available_rooms = rng.randint(cap["min"], cap["max"])
        else:
            available_rooms = rng.randint(default_cap["min"], default_cap["max"])
        base_occupied_rooms = rng.randint(int(available_rooms * 0.4), available_rooms)
        occupied_rooms = min(
            available_rooms,
            round(base_occupied_rooms * occupancy_multiplier(timestamp)),
        )
        guests = int((1 + rng.random() * 3) * occupied_rooms)
...
def build_fiscal_rows(
    ...
    sensor_count = max(100, record_count // 30)
    sensors = [f"FIS-{index + 1:04d}" for index in range(sensor_count)]
    start = datetime(2025, 1, 1, 0, 0, 0)

    fin_map = {
        "residential": {"expense": 50.0, "revenue": 250.0},
        "commercial": {"expense": 1000.0, "revenue": 2500.0},
        "industrial": {"expense": 500.0, "revenue": 1250.0},
        "mixed_use": {"expense": 100.0, "revenue": 500.0},
        "park": {"expense": 10.0, "revenue": 50.0},
        "campus": {"expense": 75.0, "revenue": 500.0},
    }
    def_map = {"expense": 10.0, "revenue": 50.0}
    ...
    for index in range(record_count):
        ...
        lat, lon = random_point(zone, rng)
        zone_type = str(zone["zone_type"])
        finfo = fin_map.get(zone_type, def_map)
        if finfo:
            expense = rng.random() * finfo["expense"]
            revenue = rng.random() * finfo["revenue"] * occupancy_multiplier(timestamp)
        else:
            expense = rng.random() * def_map["expense"]
            revenue = rng.random() * def_map["revenue"] * occupancy_multiplier(timestamp)
...
def main() -> None:
    ...
    occupancy_rows = build_occupancy_rows(args.records, zones, rng)
    fiscal_rows = build_fiscal_rows(args.records, zones, rng)
    ...
    write_csv_file(output_dir / "occupancy_data.csv", occupancy_rows)
    write_csv_file(output_dir / "fiscal_data.csv", fiscal_rows)
    ...
    print(f"Wrote {len(occupancy_rows)} rows to {output_dir / 'occupancy_data.csv'}")
    print(f"Wrote {len(fiscal_rows)} rows to {output_dir / 'fiscal_data.csv'}")
    print(
